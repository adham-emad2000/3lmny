...
function TeacherLive() {
  ...
  // دالة تشغيل صوت التنبيه عند وصول طلب جديد
  ...
  useEffect(() => {
    ...
    // الاستماع اللحظي للطلبات في نفس محافظة المعلم
    const q = query(
    ...
    const unsubscribe = onSnapshot(
      ...
      (snapshot) => {
        ...
        // تصفية الطلبات المعلقة التي تتطابق مع مادة المعلم (أو لو مادته غير مقيدة)
        ...
        // الحصص المقبولة الخاصة بهذا المعلم فقط
  ...
  const handleAcceptRequest = async (reqId, currentPrice) => {
    ...
    try {
      ...
      alert("✅ تم قبول الحصة بنجاح! تواصل مع الطالب الآن.");
    } catch (error) {
  ...
  // إنهاء الحصة وحذف الطلب من الداتابيز
  const handleFinishLesson = async (reqId) => {
    if (!window.confirm("هل أنت متأكد من إنهاء هذه الحصة وإزالة الطلب؟"))
      return;
    try {
    ...
    >
      <div className="max-w-5xl mx-auto space-y-8">
        ...
        <div className="space-y-4">
          ...
          {/* أزرار Pagination لو الطلبات أثر من 6 */}
          {totalPages > 1 && (
